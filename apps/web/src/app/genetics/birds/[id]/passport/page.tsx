  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 4, maxWidth: 700 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--neutral-400)' }}>
        ...
        <ChevronRight size={12} />
        <span style={{ color: 'var(--neutral-800)' }}>Pasaporte</span>
      </div>
      ...
      <div className="nf-card" style={{ borderTop: '4px solid var(--primary)', textAlign: 'center', padding: 24 }}>
        ...
        <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--neutral-900)', marginTop: 4 }}>{bird.anilla}</div>
        <div style={{ fontSize: 14, color: 'var(--neutral-700)' }}>{bird.nombre || 'Sin nombre'}</div>
      ...
      <div className="nf-card">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, fontSize: 12 }}>
          <div><span style={{ color: 'var(--neutral-500)' }}>Plumaje:</span> <span style={{ color: 'var(--neutral-800)' }}>{bird.colorPlumaje || '—'}</span></div>
          <div><span style={{ color: 'var(--neutral-500)' }}>Peso actual:</span> <b style={{ color: 'var(--neutral-900)' }}>{bird.pesoActual} kg</b></div>
          ...
          <div><span style={{ color: 'var(--neutral-500)' }}>Estado:</span> <span style={{ color: 'var(--neutral-800)' }}>{bird.estadoProductivo}</span></div>
          <div><span style={{ color: 'var(--neutral-500)' }}>Destino:</span> <span style={{ color: 'var(--neutral-800)' }}>{bird.destinoRecomendado?.replace(/_/g, ' ') || '—'}</span></div>
        </div>
      </div>
      ...
      <div className="nf-card">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 12 }}>
          <div style={{ padding: '6px 8px', borderRadius: 6, background: '#3B82F608' }}>
            ...
            <div style={{ color: 'var(--neutral-800)', fontWeight: 500 }}>{parents.padre ? `${parents.padre.anilla} — ${parents.padre.nombre || parents.padre.raza}` : 'Desconocido'}</div>
          </div>
          <div style={{ padding: '6px 8px', borderRadius: 6, background: '#EC489908' }}>
            <div style={{ color: 'var(--neutral-500)', fontSize: 10 }}>Madre</div>
            <div style={{ color: 'var(--neutral-800)', fontWeight: 500 }}>{parents.madre ? `${parents.madre.anilla} — ${parents.madre.nombre || parents.madre.raza}` : 'Desconocido'}</div>
          </div>
        </div>
        ...
        {Object.keys(breedContrib).length > 0 && (
          <div style={{ marginTop: 6, fontSize: 11 }}>
            ...
            {Object.entries(breedContrib).sort((a, b) => b[1] - a[1]).map(([breed, pct]) => (
              <span key={breed} style={{ color: 'var(--neutral-700)' }}>{breed} {pct.toFixed(0)}% · </span>
            ))}
          </div>
        )}
      </div>
      ...
      <div className="nf-card">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {events.map(e => (
            <div key={e.id} style={{ display: 'flex', gap: 8, fontSize: 11, padding: '4px 0', borderBottom: '1px solid var(--neutral-50)' }}>
              <span style={{ color: 'var(--neutral-500)', width: 80, flexShrink: 0 }}>{e.fecha}</span>
              <span style={{ color: 'var(--neutral-700)', fontWeight: 500, textTransform: 'capitalize' }}>{e.tipo.replace(/_/g, ' ')}</span>
              <span style={{ color: 'var(--neutral-400)' }}>{e.descripcion}</span>
            </div>
          ))}
          {measurements.filter(m => m.tipo === 'peso').map(m => (
            <div key={m.id} style={{ display: 'flex', gap: 8, fontSize: 11, padding: '4px 0', borderBottom: '1px solid var(--neutral-50)' }}>
              <span style={{ color: 'var(--neutral-500)', width: 80, flexShrink: 0 }}>{m.fecha}</span>
              <span style={{ color: 'var(--neutral-700)', fontWeight: 500 }}>Pesaje</span>
              <span style={{ color: 'var(--neutral-800)', fontWeight: 600 }}>{m.valor} kg</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {evaluation && (
        <div className="nf-card">
          ...
          {evaluation.notasOrganolepticas && (
            <div style={{ marginTop: 6, fontSize: 11, color: 'var(--neutral-700)', fontStyle: 'italic', padding: '6px 8px', borderRadius: 4, background: 'rgba(var(--primary-rgb,180,130,50),0.03)' }}>
              {evaluation.notasOrganolepticas}
            </div>
          )}
        </div>
